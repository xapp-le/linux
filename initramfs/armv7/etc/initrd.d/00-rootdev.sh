...
_rootdev_select() {
	if [ -z "${CHECK_ROOT}" ]; then
		bad_msg "No select rootdev found"
		return 1
	fi

	mkdir -p /check
	mount -t auto -o ro ${CHECK_ROOT} /check
	if [ -f /check/rootdev.txt ]; then
		REAL_ROOT="$(cat /check/rootdev.txt)"
		good_msg "Select root: ${REAL_ROOT}"
		umount /check
		return 0
	fi
	umount /check
	bad_msg "Select root fail: ${CHECK_ROOT}"
	return 1
}
